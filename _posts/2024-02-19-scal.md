...
This is a recap of the regret analysis in [SCAL](https://arxiv.org/pdf/1802.04020.pdf).
...
**Bellman operators and evaluation equations**: For the Bellman operator associated with a Markov randomized decision rule $$d$$, $$L_d$$, it satisfies that given any value function $$v \in \mathbb{R}^S$$,
...
The confidence interval $$B^k_r(s,a)$$ and $$B^k_p(s,a,s')$$ are built based on Hoeffding's Inequality (UCRL, UCRL2) or Bernstein's inequality (SCAL). The optimization problem solved by UCRL is to find $$(\tilde{M}^\star_k, \tilde{\pi}^\star_k)$$ such that
...
From UCRL2 we know that solving the above optimization/maximization problem is equivalent to finding the optimal policy $$\tilde{\pi}^\star** in the **extended MDP**(Bounded-parameter MDP),
...
REGAL.C follows the same procedure of UCRL but constrains the plausible set of MDP in a finite-span set. Formally speaking,
...
where $$\mathcal{M}_{RC}:= \{ M\in\mathcal{M}_k : sp(h^\star_M) \leq c \}$$ is the set of plausible MDP with bounded bias span for the optimal policy $$\pi^\star$$.
...
The proof idea follows the standard way used by UCRL2. The total regret will be decomposed as the sum of regret from each episode and bound each of them individually. Therefore
...
One difference between SCAL and UCRL2 is that the output policy of SCAL in each episode is a stochastic policy. Hence the definition of $$\Delta_i$$ needed to be changed. Now $$\Delta_t = g^\star - r_t(s_t, a_t)$$ instead of the cumulative regret from one episode. Next, consider at time step $$t$$, where $$k_t$$ is the corresponding episode and $$\tilde{\pi}_{k_t}$$ is the policy executed in the episode $$k_t$$, we let $$X_t:= r_t(s_t, a_t) - \sum_{a\in\mathcal{A}_{s_t}} r(s_t,a_t) \tilde{\pi}_{k_t}(s_t, a)$$ and  $$\{ X_t \}_{t=1}^T$$ becomes a Martingale Difference Sequence. Then we can use Azuma-Hoeffding's Inequality to remove the randomness of $$r_t(s_t, a_t)$$.
...
#### 5.1.3 Dealing with Following Confidence Regions
